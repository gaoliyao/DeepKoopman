...
def choose_optimizer(params, model):
    ...
    if params['opt_alg'] == 'adam':
        ...
        if params['decay_rate'] > 0:
            optimizer = torch.optim.Adadelta(model.parameters(), lr=params['learning_rate'], rho=params['decay_rate'])
        else:
            optimizer = torch.optim.Adadelta(model.parameters(), lr=params['learning_rate'])
    elif params['opt_alg'] == 'adagrad':
        optimizer = torch.optim.Adagrad(model.parameters(), lr=params['learning_rate'])
    elif params['opt_alg'] == 'RMS':
        if params['decay_rate'] > 0:
            optimizer = torch.optim.RMSprop(model.parameters(), lr=params['learning_rate'], alpha=params['decay_rate'])
        else:
